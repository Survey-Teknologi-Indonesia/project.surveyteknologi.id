...
import {
  getRawDataEnhancePageData,
  approveRawDataEnhanceGate,
  rejectRawDataEnhanceGate,
} from "./actions";
...
export default function RawDataPage({
  ...
  useEffect(() => {
    async function loadInitialData() {
      ...
      const result = await getRawDataEnhancePageData(id);
  ...
  const handleApproval = async () => {
    ...
    try {
      ...
      const res = await approveRawDataEnhanceGate(data.projectId, storedUserId);
      if (res.success) {
  ...
  const confirmRejection = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setToastMessage("Gate 4 (Raw Data Enhancement) rejected!");
      ...
      alert("An error occurred while verifying Raw Data Enhancement.");
    } finally {
      setIsRejected(false);
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3 text-slate-500">
        ...
        <span className="text-xs font-semibold">
          Loading Raw Data Enhancement (Gate 4)...
        </span>
      </div>
    );
  }
      ...
      {!hasDriveLink && (
        <div className="flex flex-col items-center justify-center gap-3 p-10 rounded-2xl bg-slate-50 border border-dashed border-slate-200 text-center">
          ...
          <p className="text-xs text-slate-400 max-w-sm">
            Upload progress for Gate 4 with a Google Drive folder link to view
            files directly here.
          </p>
        </div>
      )}
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900">
              Raw Data Enhancement Storage Folder Access
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
          ...
          >
            ...
            <span>Open Raw Data Enhancement Google Drive</span>
