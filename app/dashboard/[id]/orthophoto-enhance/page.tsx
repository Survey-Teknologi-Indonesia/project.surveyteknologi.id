...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    APPROVED: {
      label: "Orthophoto Enhance Verified",
...
export default function OrthophotoPage({
  ...
  useEffect(() => {
    async function loadInitialData() {
      ...
      if (result.success && result.data) {
        ...
        setError(
          result.error ??
            "An error occurred while fetching Orthophoto Enhancement data from the server.",
        );
      }
      setLoading(false);
    }
  ...
  const handleApproval = async () => {
    ...
    try {
      ...
      alert("An error occurred while approving Orthophoto Enhancement.");
    } finally {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3 text-slate-500">
        ...
        <span className="text-xs font-semibold">
          Loading Orthophoto Enhancement data (Gate 6)...
        </span>
      </div>
    );
  }
          ...
          {currentUser?.role === "uploader" ? (
            ...
            >
              {isApproving ? (
                ...
                <>
                  <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                  <span>Orthophoto Enhance Verified</span>
                </>
              ) : (
                <span>Approve Orthophoto Enhance</span>
              )}
            </button>
          )}
        </div>
      ...
      {!hasDriveLink && (
        <div className="flex flex-col items-center justify-center gap-3 p-10 rounded-2xl bg-slate-50 border border-dashed border-slate-200 text-center">
          ...
          <p className="text-xs text-slate-400 max-w-sm">
            Upload progress on Gate 6 with a Google Drive folder URL to view
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h3 className="text-sm font-bold text-slate-900">
              Access Orthophoto Enhance Storage Folder
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              All enhanced orthophoto files are organized in Google Drive and
              ready for download or GIS integration.
            </p>
          </div>
