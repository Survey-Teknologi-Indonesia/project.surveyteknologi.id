...
export interface DataAcquisitionPageData {
  projectId: string;
  projectName: string;
  client: string;
  gateNumber: number;
...
export async function getDataAcquisitionPageData(projectId: string): Promise<{
  success: boolean;
  data?: DataAcquisitionPageData;
  error?: string;
}> {
  try {
    // 1. Fetch step info Gate 5 (Orthophoto)
    const stepRes = await pool.query(
      "SELECT step_id, TRIM(step_name) AS step_name, step_number FROM step WHERE step_number = 2"
    );
    ...
    // Determine status
    let status: DataAcquisitionPageData["status"] = "NOT_UPLOADED";
    if (progressRow) {
...
// ─── Approval Actions ─────────────
export async function approveDataAcquisitionGate(
  projectId: string,
  approverId?: string
  ...
  try {
    ...
    const stepRes = await client.query(
      "SELECT step_id FROM step WHERE step_number = 2"
    );
    if (stepRes.rows.length === 0) {
      await client.query("ROLLBACK");
      return { success: false, message: "Gate 2 stage not found in database." };
    }
    const stepId = stepRes.rows[0].step_id;
...
export async function rejectDataAcquisitionGate(
  projectId: string,
  rejectorId?: string,
  reason?: string,
): Promise<{ success: boolean; message?: string }> {
  ...
  try {
    ...
    revalidatePath(`/dashboard/${projectId}/data-acquisition`);
    return { success: true, message: "Data Acquisition successfully rejected!" };
  } catch (error: any) {
    await client.query("ROLLBACK");
    console.error("Approval error:", error);
    return {
